---
title: "Organizing ENIGH Household Data for 2010"
...
df_hshd <-read_csv(paste0(path.data, "2010_enigh_hogares.csv"), col_names = TRUE,
...
```{r}
colnames(household)[colnames(household)=="claviv"] <-"tipo_viv"
colnames(household)[colnames(household)=="combus"] <-"combustible"
colnames(household)[colnames(household)=="elect"] <- "disp_elect"
colnames(household)[colnames(household)=="hogares2"] <- "tot_hog"
colnames(household)[colnames(household)=="residentes"] <- "tot_resid"
colnames(household)[colnames(household)=="estim"] <- "estim_pago"
colnames(household)[colnames(household)=="antigua"] <- "antiguedad"
colnames(household)[colnames(household)=="pisos"] <- "mat_pisos"
colnames(household)[colnames(household)=="pared"] <- "mat_pared"
colnames(household)[colnames(household)=="cuart"] <- "num_cuarto"

colnames(household)[colnames(household)=="eqh17_n"] <- "num_compu"
colnames(household)[colnames(household)=="eqh17_a"] <- "anio_compu"

colnames(household)[colnames(household)=="eqh8_n"] <- "num_tosta"
colnames(household)[colnames(household)=="eqh8_a"] <- "anio_tosta"

colnames(household)[colnames(household)=="eqh9_n"] <- "num_micro"
colnames(household)[colnames(household)=="eqh9_a"] <- "anio_micro"

colnames(household)[colnames(household)=="eqh10_n"] <- "num_refri"
colnames(household)[colnames(household)=="eqh10_a"] <- "anio_refri"

colnames(household)[colnames(household)=="eqh7_n"] <- "num_licua"
colnames(household)[colnames(household)=="eqh7_a"] <- "anio_licua"
colnames(household)[colnames(household)=="eqh4_n"] <- "num_tv"
colnames(household)[colnames(household)=="eqh4_a"] <- "anio_tv"
colnames(household)[colnames(household)=="eqh12_n"] <- "num_lavad"
colnames(household)[colnames(household)=="eqh12_a"] <- "anio_lavad"
colnames(household)[colnames(household)=="focos"] <- "focos_tot"
colnames(household)[colnames(household)=="tenen"] <- "tenencia"
colnames(household)[colnames(household)=="lavad"] <- "lavadero"
colnames(household)[colnames(household)=="calen"] <- "calentador"
colnames(household)[colnames(household)=="calef"] <- "heat"
colnames(household)[colnames(household)=="aire_aco"] <- "ac"
...
Notes: ENIGH 2010_The variables esc_radio, er_aparato er_celular, 
         er_compu, er_aplicac, er_tv, er_otro, recib_tvd
         Also instead of having 2 attributes to collect data related to analogue 
         and digital tvs (anio_tva, anio_tvd, num_tva, num_tvd) like in ENIGH 
         2014-2018 this dataset only includes anio_tv and num_tv

```{r}
df_hshd <- as_tibble(df_hshd) %>%
  select("folioviv", "foliohog", "huespedes", "huesp_come", "num_trab_d", 
         "trab_come", "telefono", "celular", "tv_paga", "conex_inte", 
         "num_auto", "anio_auto", "num_van", "anio_van", "num_pickup", 
         "anio_pickup", "num_moto", "anio_moto", "num_bici", "anio_bici", 
         "num_trici", "anio_trici", "num_carret", "anio_carret", "num_canoa", 
         "anio_canoa", "num_otro", "anio_otro", "num_ester", "anio_ester", 
         "num_grab", "anio_grab", "num_radio", "anio_radio", "num_tv", 
         "anio_tv", "num_dvd", "anio_dvd", "num_video", 
         "anio_video", "num_licua", "anio_licua", "num_tosta", "anio_tosta", 
         "num_micro", "anio_micro", "num_refri", "anio_refri", "num_estuf", 
         "anio_estuf", "num_lavad", "anio_lavad", "num_planc", "anio_planc", 
         "num_maqui", "anio_maqui", "num_venti", "anio_venti", "num_aspir", 
         "anio_aspir", "num_compu", "anio_compu", "num_impre", "anio_impre", 
         "num_juego", "anio_juego")
...
df_hshd <- subset(df_hshd, select= c(-state_code, -urbanization, -ten_surv, -upm,  -consecutive_dwell))

write.csv(df_hshd, file = 'outputs/2010_enigh_households.csv', na = "NULL", 
